			...
			<main className={`padding-top-10rem`}>
...

				{
					props.isAuthenticated ? (<div className={`loginRedirectMsg`}>
						...
						{ typeof window !== 'undefined' && window.setTimeout(() => {router.push('/admin')}, 4000)}
					</div>) : (<Fragment>
								...
								<div>
									...
									<div>
										...
										<div className={styles.usernameFC}>
											...
											<input type="text" name="username" value={username} onChange={handleInputChange} required={true} />
											{ /* simple validation */
												simpleValidator.current.message('username', username, 'required|alpha_num|between:4,25')
											}
										</div>
										<div className={styles.passwordFC}>
											<label htmlFor="">Enter Password</label>
											<input type="password" name="password" value={password} onChange={handleInputChange} required={true} />
											{ /* simple validation */
												simpleValidator.current.message('password', password, 'required|between:4,25')
											}
										</div>
										<div className={styles.submitFC}>
											<input type="submit" name="submit" value={props.isAttemptingLogin ? "Attempting Login..." : "Log in"} />
										</div>
									</div>
