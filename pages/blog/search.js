...
const Search = (props) => {
	...
	useEffect(() => {
		if (searchValue.length > 0) {
			setSearchedPosts(allPosts.filter(post => post.title.toLowerCase().includes(searchValue) || post.body.toLowerCase().replace(/[*#`]/g, "").substr(0, 196).includes(searchValue )));
		} else {
			setSearchedPosts([]);
		}
	}, [searchValue])
				...
				<div className="container-max-1248px">
					{
						...
						!props.post.isLoaded ? (<div>
							...
							<div className={styles.searchInputWrapper}>
								<input type="text" value={searchValue} onChange={handleSearchInput} placeholder="Search articles..." />							
							</div>
							{ searchValue.length > 0 && <p className={styles.resultCount}>{
								searchedPosts.length <= 1 ? <span><b>{searchedPosts.length}</b> result found.</span> : <span><b>{searchedPosts.length}</b> results found.</span>
						}</p>}
