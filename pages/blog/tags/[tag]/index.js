...
const Tags = (props) => {
	...
	return (
		<Fragment>
			...
			/>
			<Head>
				<title>{!props.post.isLoaded ? 'Loading...' : (allPostsWithTag.length === 0) ? "404 Error" : `Blog (Tag: #${tag}) - Gideon Idoko`}</title>
			</Head>
				...
				<div className="container-max-1248px">
					{
						...
						!props.post.isLoaded ? (<div>
							...
							<h2 style={{marginBottom: "4rem"}}>#{tag} &nbsp; <small>({allPostsWithTag.length <= 1 ? `${allPostsWithTag.length} post` : `${allPostsWithTag.length} posts` })</small> </h2>
