            ...
            <div className="container-max-1248px">
              <Fragment>
                <h2 style={{ marginBottom: '4rem' }}>
                  #{tag} &nbsp;{' '}
                  <small>({posts?.totalDocs <= 1 ? `${posts?.totalDocs} post` : `${posts?.totalDocs} posts`})</small>{' '}
                </h2>
                  ...
                  <div className={styles.pagination}>
                    <span>
                      {posts?.hasPrevPage && (
                        <Link
                          href={`/blog/tags/${tag}${
                            Number(posts?.page) === 2 ? '' : `/page/${Number(posts?.page) - 1}`
                          }`}
                        >
                          ← Previous Page
                        </Link>
                    ...
                    <span>
                      {posts?.hasNextPage && (
                        <Link href={`/blog/tags/${tag}/page/${Number(posts?.page) + 1}`}>Next Page →</Link>
                      )}
                    </span>
                  </div>
                </div>
