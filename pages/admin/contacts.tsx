                      ...
                      <li key={contact._id}>
                        <div className={styles.contactHead}>
                          <span>
                            ...
                            <small>{moment(contact.created_at).format('D/M/YYYY, h:m a')}</small>]
                          </span>
                          <span>
                            <button className={styles.toggleMessageBtn} onClick={() => toggleMessage(contact._id)}>
                              <i className={shownId.includes(contact._id) ? 'neu-minus-circle' : 'neu-add-circle'}></i>
                            </button>
                            <button
                              onClick={() =>
                                handleContactDelete({
                  ...
                  {contacts ? (
                    <div className={styles.paginationWrapper}>
                      <p className={styles.pageStats}>
                        Page {contacts?.page} of {contacts?.totalPages} {contactsLoading && <>(Loading...)</>}
                      </p>
                      <div className={styles.pagination}>
                        <span>
                          {contacts?.hasPrevPage && (
                            <button onClick={() => fetchContacts(Number(contacts?.page) - 1)}>← Previous Page</button>
                          )}
                        </span>
                        <span>
                          {contacts?.hasNextPage && (
                            <button onClick={() => fetchContacts(Number(contacts?.page) + 1)}>Next Page →</button>
                          )}
                        </span>
                      </div>
                    </div>
