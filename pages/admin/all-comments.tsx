                      ...
                      {posts?.docs.map((post) => (
                        <li key={post._id}>
                          <div className={styles.postHead}>
                            <span>
                              {post.title} [<small>Comments:({post.comments.length})</small>]
                            </span>

                            <span>
                              <Link href={`/blog/${post.slug}`} className={styles.externalLink}>
                                <i className="neu-external-link"></i>
                              </Link>
                              <button className={styles.togglePostBtn} onClick={() => togglePost(post._id)}>
                                <i className={shownId.includes(post._id) ? 'neu-minus-circle' : 'neu-add-circle'}></i>
                              </button>
                            </span>
                          </div>
                              ...
                              {post.comments.map((comment) => (
                                <div className={styles.singleComment} key={comment._id}>
                                  <div className={styles.commentHead}>
                                    ...
                                    <span>
                                      <button className={styles.togglePostBtn} onClick={() => togglePost(comment._id)}>
                                        <i
                                          className={
                                            shownId.includes(comment._id) ? 'neu-minus-circle' : 'neu-add-circle'
                                          }
                                        ></i>
                                      </button>
                                    </span>
                                  </div>
                                  {shownId.includes(comment._id) && comment.replies.length !== 0 ? (
                                    <div className={styles.commentBody}>
                                      {comment.replies.map((reply) => (
                                        <div className={styles.singleReply} key={reply?._id}>
                                          <b>{reply.reply_author}</b>:{' '}
                                          <span dangerouslySetInnerHTML={createMarkup(reply.reply_body)} />
                                        </div>
                                      ))}
                                    </div>
                                  ) : shownId.includes(comment._id) && comment.replies.length === 0 ? (
                                    <div className={`${styles.commentBody} ${styles.p05}`}>No Comments</div>
                                  ) : null}
                                </div>
                              ))}
                            </div>
                  ...
                  {posts ? (
                    <div className={styles.paginationWrapper}>
                      <p className={styles.pageStats}>
                        Page {posts?.page} of {posts?.totalPages} {postLoading && <>(Loading...)</>}
                      </p>
                      <div className={styles.pagination}>
                        <span>
                          {posts?.hasPrevPage && (
                            <button onClick={() => fetchComments(Number(posts?.page) - 1)}>← Previous Page</button>
                          )}
                        </span>
                        <span>
                          {posts?.hasNextPage && (
                            <button onClick={() => fetchComments(Number(posts?.page) + 1)}>Next Page →</button>
                          )}
                        </span>
                      </div>
                    </div>
