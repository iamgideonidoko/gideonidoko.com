				...
				{ /*UPLOAD ASSET PAGE*/}
					<div className={styles.uploadAssetWrap}>
						...
						<form className={styles.fileUploadForm}>
							<label htmlFor="fileUploadInput" className={styles.uploadInitiator}><i className="neu-up-md"></i></label><br />
							<input type="file" id="fileUploadInput" className={styles.fileUploadInput} onChange={fileChangeHandler} />
							<div className={styles.output}>
								{ error && <div className={styles.uploadErrorMsg}>{ error }</div>}
								{ file && <div className={styles.selectFileName}>Selected file: { file.name }</div> }
								{ (file && startDownload) ? <ProgressBar file={file} setFile={setFile} setStartDownload={setStartDownload} /> : null }
