...
const ThemeSwitch = (props) => {
	...
	useEffect(() => {
		setTheme(window.localStorage.getItem('gideonidoko.com-theme') ? window.localStorage.getItem('gideonidoko.com-theme') : 'dark')
	}, [])
	...
	return (
		<div className={(!props.isNavOpen && props.allowForMobile) ? "themeswitch-wrapper" : "themeswitch-wrapper navOpen"}>
			<button style={{zIndex: '50000'}} onClick={handleSwitch}><i className={theme === 'dark' ? 'neu-moon-stars' : 'neu-sun'}></i></button>
		</div>
	)
}
