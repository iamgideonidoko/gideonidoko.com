...
export const getPosts = () => dispatch => {
	...
	axios.get(`${config.BEHOST}/api/blogposts`)
		.then(res => dispatch({
...
export const addPost = (newPost) => (dispatch, getState) => {
	...
	axios.post(`${config.BEHOST}/api/blogposts`, requestBody, tokenConfig(getState))
		.then(res => {
			dispatch({
				type: types.ADD_POST,
				payload: res.data
...
export const updatePost = (postId, updatedPost) => (dispatch, getState) => {
	...
	axios.put(`${config.BEHOST}/api/blogposts/${postId}`, requestBody, tokenConfig(getState))
		.then(res => {
			dispatch({
...
export const updatePostComments = (postId, updatedPost) => (dispatch, getState) => {
	const requestBody = JSON.stringify(updatedPost);
	axios.put(`${config.BEHOST}/api/blogposts/commentsupdate/${postId}`, requestBody, tokenConfig(getState))
		.then(res => {
			dispatch({
				type: types.UPDATE_POST,
				payload: res.data
...
export const deletePost = (postId) => (dispatch, getState) => {
	...
	axios.delete(`${config.BEHOST}/api/blogposts/${postId}`, tokenConfig(getState))
		.then(res => {
			dispatch({
