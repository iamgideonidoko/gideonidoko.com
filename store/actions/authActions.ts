...
export const loadAdminUser = () => (dispatch, getState) => {
    //getState fetches the current statue  in the reducer store

    dispatch({ type: types.ADMIN_USER_LOADING });

    axios
        .get(`${config.BEHOST}/api/auth/user`, tokenConfig(getState))
        .then((res) =>
            dispatch({
...
export const login =
    ...
    (dispatch) => {
        ...
        axios
            .post(`${config.BEHOST}/api/auth`, body, headerConfig)
            .then((res) =>
                dispatch({
