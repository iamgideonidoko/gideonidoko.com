...
export const separatedStrToArr = (str) => {
	...
	const resArr = splitStr.filter(x => {if (x) return x;});
	return resArr;
}
...
export const getReadTime = string => {
	...
	return (roundedMinutes < 1) ? seconds <= 1 ? seconds + " sec read" : seconds + " secs read" : roundedMinutes <= 1 ? roundedMinutes + " min read" : roundedMinutes + " mins read";
	
}

export const commaSeparatedStr = (tagArr) => {
	const mappedTagArr = tagArr.map(x => x.replace(/\W/ig, '_'));
